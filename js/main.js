...
function isTouchDevice() {
	return ('ontouchstart' in document.documentElement) && (
		navigator.userAgent.match(Androidi)
		 navigator.userAgent.match(webOSi)
		 navigator.userAgent.match(BlackBerryi)
		 navigator.userAgent.match(Windows Phonei)
		 navigator.userAgent.match(Opera Minii)
		 navigator.userAgent.match(IEMobilei)
		 navigator.userAgent.match(iPhonei)
		 navigator.userAgent.match(iPadi)
		 navigator.userAgent.match(iPodi)
...
$(function() {
	...
	(function() {
		...
		$('.wb_video_background').each(function() {
			...
			if (url) {
				...
				if (youtubeVideoId) {
					if (!window.YT) {
						$.getScript('httpswww.youtube.comiframe_api');
					}
					...
					window.onYouTubeIframeAPIReady = function() {
						...
						var resizer = function() {
							...
							if (w  h  169) {
								youtube.setSize(w, w  169);
								innerCont.css('left', 0);
							} else {
								youtube.setSize(h  916, h);
	...
	(function() {
		...
		if (menu.length) {
			...
			menu.find('a').on('click', function() {
				...
				if (^(httphttps).test(href)) return true;
	...
	(function() {
		...
		if (header.length) {
			...
			if (firefoxedgi.test(navigator.userAgent)) {
				var pageAnchor, p, el;
		...
		var updatePositionFixed = function() {
			if (isTouchDevice()) {
				...
				for (i = 0; i  rules.length; i++) {
					if (rules[i].selectorText === 'body.sitebefore') {
						if (bfixedbi.test(rules[i].cssText)) {
				...
				if (fixedBgCss) {
					var css = fixedBgCss.match({(.+)})[1].replace(bfixedbi, 'scroll');
					css += ' position fixed';
